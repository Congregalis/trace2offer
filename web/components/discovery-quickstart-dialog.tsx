...
import { DISCOVERY_PRESETS, getDiscoveryPresetsByGroup } from "@/lib/discovery-presets";
import { Dialog, DialogContent, DialogDescription, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Badge } from "@/components/ui/badge";

export function DiscoveryQuickstartDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>发现规则快速上手</DialogTitle>
          <DialogDescription>先用推荐示例跑起来，再慢慢收关键词，别一上来把自己填进沟里。</DialogDescription>
        </DialogHeader>

        <div className="space-y-5 text-sm">
          <section className="space-y-2">
            <h3 className="font-medium text-foreground">1. 发现规则是干啥的</h3>
            <p className="text-muted-foreground">
              一条发现规则就是告诉系统去哪个 RSS/Atom 源抓岗位，再用包含词和排除词决定哪些岗位进候选池。
            </p>
          </section>

          <section className="space-y-2">
            <h3 className="font-medium text-foreground">2. 新手最稳的开局</h3>
          ...
          <section className="space-y-2">
            <h3 className="font-medium text-foreground">3. 推荐先用的规则</h3>
          ...
          <section className="space-y-2">
            <h3 className="font-medium text-foreground">4. 每个字段怎么理解</h3>
          ...
          <section className="space-y-2">
            <h3 className="font-medium text-foreground">5. 推荐关键词起步模板</h3>
            <div className="space-y-3 rounded-xl border border-border bg-card/30 p-4">
              <div>
          ...
          <section className="space-y-2">
            <h3 className="font-medium text-foreground">6. 别踩这些坑</h3>
            <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
          ...
          <section className="text-xs text-muted-foreground">
            当前应用内示例和仓库里的 `docs/discovery-rules-quickstart.md` 保持同一套 feed 和关键词事实源。
          </section>
        </div>
      </DialogContent>
    </Dialog>
